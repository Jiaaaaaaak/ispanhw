...
const FOOD_DATA = [
    ...
    { name: "義大利麵", priceRange: "80-140", category: "noodle", tag: "麵類" },
    { name: "拉麵", priceRange: "120-140", category: "noodle", tag: "麵類" },
...
async function searchDelivery(keyword) {
    try {
        const { lat, lng } = await getUserLocation();

        const res = await fetch(`http://127.0.0.1:5000/api/delivery?keyword=${keyword}&lat=${lat}&lng=${lng}`);
        const data = await res.json();

        displayDeliveryList(data.restaurants);
    } catch (err) {
        console.error("外送平台搜尋失敗：", err);
    }
}
...
function displayDeliveryList(restaurants) {
    const box = document.getElementById("delivery-list");
    box.innerHTML = "";

    if (!restaurants || restaurants.length === 0) {
        box.innerHTML = `<p>附近沒有找到餐點 😢</p>`;
        return;
    }
    ...
    restaurants.forEach(r => {
        ...
        const img = r.image ? r.image : "default.jpg";

        div.innerHTML = `
            <img class="delivery-img" src="${img}" />
            <div class="delivery-info">
                ...
                <a class="delivery-link" href="${r.link}" target="_blank">查看餐廳</a>
            </div>
        `;
